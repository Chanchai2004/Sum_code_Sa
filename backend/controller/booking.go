...
func BookSeats(c *gin.Context) {
	...
	for _, seat := range seats {
		...
		// เพิ่มการบันทึกความสัมพันธ์ใน book_seat table
		...
		if err := tx.Create(&bookSeat).Error; err != nil {
			tx.Rollback()
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create book_seat record"})
			return
		}
	}
...
func ReleaseSeatsForUnfinishedTickets(c *gin.Context) {
	...
	for _, ticket := range tickets {
		...
		// สร้าง slice เพื่อเก็บ BookingID จาก bookings
		var bookingIDs []uint
		for _, booking := range bookings {
			bookingIDs = append(bookingIDs, booking.ID)
		}

		// ลบข้อมูลความสัมพันธ์ใน book_seat ก่อนลบ booking
		if err := db.Where("booking_id IN ?", bookingIDs).Delete(&entity.BookSeat{}).Error; err != nil {
			log.Println("Failed to delete book_seat records for ticket:", ticket.ID)
			c.JSON(500, gin.H{"error": "Failed to delete book_seat records for ticket"})
			return
		}
